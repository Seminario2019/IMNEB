...
public class usuario_crud {
  ...
  public JSONArray login(String usuario, String Clave) {
    ...
    try {
      ...
      ResultSet rs = stmt.executeQuery("SELECT rtrim(id_user) id_user, rtrim(usuario) usuario,clave,rtrim(nombre)+' '+rtrim(apellido) as nombre FROM usuarios where usuario  =  '"+usuario+"' and clave =  '"+Clave+"'");
      while(rs.next()){
