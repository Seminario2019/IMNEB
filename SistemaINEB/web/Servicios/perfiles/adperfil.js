...
app.controller("adperfil", function($scope,$http,$location,serveData,uploadfoto,md5) {
        ...
        $scope.uploadFile = function(a)
	{
		...
		{       $scope.dato.imagen =  $scope.dato.usuario;
			console.log(res);
                        if(res.data == "") swal("Error","Imagen no  subio","warning");
                             ...
                             $scope.intc++;
                        $scope.imagen="Servicios/perfiles/img_usuarios/"+$scope.dato.imagen+"/imagen.jpg?1."+$scope.intc;    
                            }
		})
	}
        ...
        $scope.limpia = function(){
            $scope.dato.nombre = "";  $scope.dato.apellido = "";  $scope.dato.email = ""; 
                $scope.dato.pais = "";  $scope.dato.depto = "";  $scope.dato.coments = ""; 
                 $scope.dato.edad = "";  $scope.dato.sexo = "";  $scope.dato.telefono = ""; 
                  $scope.dato.celular = "";  $scope.dato.cede = "";  $scope.dato.bio = ""; 
        ...
        $scope.guardar = function refreshData() {
           $http.post("Servicios/perfiles/adperfilSave.php"
            ,{datos:$scope.dato})
             .success(function(response){
             //$window.alert(response[0].mensaje);*/swal( 'Good job!', 'You clicked the button!', 'success')
              if(response[0].mensaje == 'uno'){
                  ...
                  $scope.editado = {
                    ...
                    $http.get("Servicios/perfiles/adperfil.php").success(function(data){
                       $scope.list= data; 
                       $scope.paginas();
                    });
              }
